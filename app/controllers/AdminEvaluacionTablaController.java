...
public class AdminEvaluacionTablaController extends ControladorSeguroCoordinador {
    ...
    public static Result ajaxDeleteReactivo() throws JSONException{
        Logger.debug("Desde AdminEvaluacionTablaController.ajaxDeleteReactivo...");
        JsonNode json = request().body().asJson();
        System.out.println(json);
        JSONObject retorno = new JSONObject();
        try {
            EvaluacionTablaReactivo.find.byId(json.findValue("id").asLong()).delete();
            retorno.put("estado", "eliminado");
        } catch(Exception e){
            System.out.println("ERRRRORRRR "+e.getMessage());
            System.out.println("ERRRRORRRR "+e.getCause());
            retorno.put("estado","error");
            retorno.put("errorDescripcion","No se puede eliminar un reactivo que esté en alguna versión del instrumento de evaluación.");
        }
...
//System.out.println("ajax - options "+ cadena );
    ...
    public static Result ajaxEditTablaEvaluacionCriterio(){
        System.out.println("¸\n\n\n\n...........................   desde AdminEvaluacionTablaController.ajaxEditTablaEvaluacionCriterio ");
        EvaluacionTabla obj = null;
        JsonNode json = request().body().asJson();
        System.out.println(json);
        ...
        try {
            ...
            EvaluacionTabla evt = EvaluacionTabla.find.where()
                    ...
                    .findUnique();
            System.out.println("001");
            System.out.println("+obj.aspecto.id  "+obj.aspecto.id);
            ...
            //obj.update();
            System.out.println("002");
            System.out.println("id de la versionactual del instrumento: "+obj.version.id);
            System.out.println("002 - 001");

            EvaluacionTablaVersion evtv = EvaluacionTablaVersion.find.where().eq("id", obj.version.id).findUnique();

            System.out.println("evtv: "+evtv);
            System.out.println("002 - 002");


            System.out.println("003");
            if (Objects.equals(borrado, "false"))
    ...
    public static Result ajaxEditTablaEvaluacionTipoRecurso(){
        System.out.println("\n\n\n\n...........................   desde AdminEvaluacionTablaController.ajaxEditTablaEvaluacionTipoRecurso ");
...
//        System.out.println("id EvaluacionTabla "+evt.id);
    ...
    public static Result reactivosDTSS(){
        Logger.info("\n\n\nDesde AdminEvaluacionTablaController.reactivoDTSS");
        JSONObject json2 = new JSONObject();
        int filtrados = 0;
        int sinFiltro = 0;
        Map<Integer, Integer> columnasOrdenables = new HashMap<>();
        ...
        Page<EvaluacionTablaReactivo> reacs = null;

        Query<EvaluacionTablaReactivo> q1 = EvaluacionTablaReactivo.find;
        Query<EvaluacionTablaReactivo> q2 = EvaluacionTablaReactivo.find.where(
        ...
        reacs = EvaluacionTablaReactivo.find
                .where(
                        "descripcion like :cadena "
                ).setParameter("cadena", "%"+filtro+"%")
                // .orderBy( "c"+    (colOrden==0?columnasOrdenables.get(0)  :  columnasOrdenables.get(colOrden)-1)  +" "+tipoOrden )
